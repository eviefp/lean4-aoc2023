...
def numberOfVariants: List Spring -> List Nat -> Bool -> Nat
  ...
  | Spring.Operational :: _, _, true => 0 -- if we were inside a group but got a wrong entry, stop
  | Spring.Damaged :: _, 0 :: _, _ => 0
  ...
  | Spring.Unknown :: xs, (k + 1) :: ys, false => -- we diverge!
      ...
      if maxDamaged < neededDamaged
        then 0 -- impossible to fulfil, don't diverge
        else
            Nat.add
                (numberOfVariants xs ((k + 1) :: ys) false) -- use operational
                (numberOfVariants xs ((k) :: ys) true)  -- use broken
  | [], [0], _ => 1
  | [], [], _ => 1
  | _, _, _ => 0

def solve: List Input -> Nat :=
  Monoid.Nat.Sum.Instance.foldMap
    (λ i => numberOfVariants i.record i.series false)
...
def main: IO Unit := do
  ...
  let input2 := Sample.input1.map updateInput

  IO.println s!"part2 {solve input2}"
